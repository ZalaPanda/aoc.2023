CLASS zcl_day10 DEFINITION PUBLIC FINAL CREATE PUBLIC.
  PUBLIC SECTION.
...
CLASS zcl_day10 IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.
    DATA(result1) = zcl_day10=>part1( ).
    out->write(
        name = |The steps along the loop does it take to get from the starting position to the point farthest from the starting position:|
        data = result1 ). " Answer: 6754

    DATA(result2) = zcl_day10=>part2( ).
    out->write( name = |The tiles enclosed by the loop:|
                data = result2 ). " Answer: 567
  ENDMETHOD.

  METHOD part1.
    DATA(input) = COND string( WHEN use_sample = abap_true THEN get_puzzle_sample( ) ELSE get_puzzle_input( ) ).
    DATA(width) = COND i( WHEN use_sample = abap_true THEN 5 ELSE 140 ).
    SPLIT '|N |S -E -W LN LE JN JW 7S 7W FS FE' AT ' ' INTO TABLE DATA(pipes).
    TYPES: BEGIN OF move_type,
             direction TYPE c LENGTH 1,
             opposite  TYPE c LENGTH 1,
             offset    TYPE i,
           END OF move_type.
    TYPES move_table TYPE TABLE OF move_type WITH EMPTY KEY.
    DATA(moves) = VALUE move_table( ( direction = 'N' opposite = 'S' offset = width * -1 )
                                    ( direction = 'E' opposite = 'W' offset = 1 )
                                    ( direction = 'S' opposite = 'N' offset = width )
                                    ( direction = 'W' opposite = 'E' offset = -1 ) ).

    DATA(tile) = 'S'.
    DATA(current_offset) = 0.
    DATA(animal_offset) = find( val = input
                                sub = tile ).

    DATA(move) = VALUE move_type( ).
    LOOP AT moves INTO move. " NOTE: get start direction, the first one is fine
      current_offset = animal_offset + move-offset.
      tile = input+current_offset(1).
      IF line_exists( pipes[ table_line = |{ tile }{ move-opposite }| ] ).
        EXIT.
      ENDIF.
    ENDLOOP.

    WHILE NOT tile = 'S'.
      LOOP AT pipes ASSIGNING FIELD-SYMBOL(<pipe>).
        IF <pipe>+0(1) = tile AND <pipe>+1(1) <> move-opposite.
          move = moves[ direction = <pipe>+1(1) ].
          EXIT.
        ENDIF.
      ENDLOOP.
      current_offset = current_offset + move-offset.
      tile = input+current_offset(1).
      result = result + 1.
    ENDWHILE.
  ...
  METHOD part2.
    ...
    TYPES: BEGIN OF move_type,
             direction TYPE c LENGTH 1,
             opposite  TYPE c LENGTH 1,
             beside    TYPE c LENGTH 1, " NOTE: the left side; right side could work also
             offset    TYPE i,
           END OF move_type.
    TYPES move_table TYPE TABLE OF move_type WITH EMPTY KEY.
    DATA(moves) = VALUE move_table( ( direction = 'N' opposite = 'S' beside = 'W' offset = width * -1 )
                                    ( direction = 'E' opposite = 'W' beside = 'N' offset = 1 )
                                    ( direction = 'S' opposite = 'N' beside = 'E' offset = width )
                                    ( direction = 'W' opposite = 'E' beside = 'S' offset = -1 ) ).

    result = strlen( input ).
    DATA(current_offset) = 0.
    DATA(beside_offset) = 0.
    DATA(animal_offset) = find( val = input
                                sub = 'S' ).

    DATA(tile) = '?'.
    DATA(move) = VALUE move_type( ).
    LOOP AT moves INTO move.
      current_offset = animal_offset + move-offset.
      tile = input+current_offset(1).
      IF NOT line_exists( pipes[ table_line = |{ tile }{ move-opposite }| ] ).
        CONTINUE.
      ENDIF.

      TYPES hashed_integer_table TYPE HASHED TABLE OF i WITH UNIQUE DEFAULT KEY.
      DATA(path_offsets) = VALUE hashed_integer_table( ( current_offset ) ).
      DATA(beside_offsets) = VALUE hashed_integer_table( ). " NOTE: the offset numbers along the path to the left
      " HINT: actually, this is incorrect because the calculation for `beside_offset` should also be included here, but it still produces the correct result for me
      ...
      WHILE NOT tile = 'S'.
        ...
        beside_offset = current_offset + moves[ direction = move-beside ]-offset.
        IF beside_offset BETWEEN 1 AND strlen( input ) AND ( current_offset DIV width = beside_offset DIV width OR current_offset MOD width = beside_offset MOD width ).
          INSERT beside_offset INTO TABLE beside_offsets.
        ENDIF.

        current_offset = current_offset + move-offset.
        tile = input+current_offset(1).
        INSERT current_offset INTO TABLE path_offsets.
      ...
      beside_offsets = FILTER hashed_integer_table( beside_offsets EXCEPT IN path_offsets WHERE table_line = table_line ).

      LOOP AT beside_offsets INTO current_offset.
        LOOP AT moves INTO move.
          beside_offset = current_offset + move-offset.
          IF beside_offset BETWEEN 1 AND strlen( input ) AND ( current_offset DIV width = beside_offset DIV width OR current_offset MOD width = beside_offset MOD width ).
            IF NOT line_exists( path_offsets[ table_line = beside_offset ] ).
              INSERT beside_offset INTO TABLE beside_offsets. " NOTE: this does not look good and DRY!
            ENDIF.
          ENDIF.
        ENDLOOP.
      ENDLOOP.

      result = nmin( val1 = result
                     val2 = lines( beside_offsets ) ).
    ENDLOOP.
  ENDMETHOD.
