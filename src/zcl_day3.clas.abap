CLASS zcl_day3 DEFINITION PUBLIC FINAL CREATE PUBLIC.
  PUBLIC SECTION.
...
CLASS zcl_day3 IMPLEMENTATION.
  METHOD part1.
    DATA(input) = SWITCH string( use_sample WHEN abap_true THEN get_puzzle_sample( ) ELSE get_puzzle_input( ) ).
    DATA(line_width) = SWITCH i( use_sample WHEN abap_true THEN 10 ELSE 140 ).
    DATA(line_count) = strlen( input ) DIV line_width.
    DATA(offset) = 0.
    WHILE offset < line_count * line_width.
      IF NOT input+offset(1) BETWEEN '0' AND '9'.
        offset = offset + 1.
        CONTINUE.
      ENDIF.
      ...
      DATA(number) = 0.
      DO.
        number = number * 10 + CONV i( input+offset(1) ).
        offset = offset + 1.
        length = length + 1.
        IF offset MOD line_width = 0.
          EXIT.
        ENDIF.
        IF input+offset(1) NOT BETWEEN '0' AND '9'.
          EXIT.
        ENDIF.
      ENDDO.

      DATA(row_begin) = ( offset - 1 ) DIV line_width
        - COND i( WHEN offset > line_width THEN 1 ELSE 0 ).
      DATA(row_while) = ( offset - 1 ) DIV line_width
        + COND i( WHEN offset < line_width * ( line_count - 1 ) THEN 1 ELSE 0 ).
      DATA(col_begin) = ( offset - length ) MOD line_width
        - COND i( WHEN ( offset - length ) MOD line_width > 0 THEN 1 ELSE 0 ).
      DATA(col_while) = ( offset - 1 ) MOD line_width
        + COND i( WHEN offset MOD line_width > 0 THEN 1 ELSE 0 ).
      TYPES integer_array TYPE TABLE OF i WITH EMPTY KEY.
      DATA(positions) = VALUE integer_array( FOR row = row_begin WHILE row <= row_while
                                             FOR col = col_begin WHILE col <= col_while
                                             ( row * line_width + col ) ).

      DATA(is_pn) = abap_false.
      LOOP AT positions INTO DATA(position).
        IF NOT input+position(1) BETWEEN '0' AND '9' AND NOT input+position(1) = '.'.
          is_pn = abap_true.
          EXIT.
        ENDIF.
      ENDLOOP.
      ...
      ENDIF.
    ENDWHILE.
    WRITE / |{ result } is the sum of all of the part numbers in the engine schematic|.
  ENDMETHOD.

  METHOD part2.
    TYPES numbers_table TYPE TABLE OF i WITH EMPTY KEY.
    TYPES: BEGIN OF gear_type,
             position TYPE i,
             numbers  TYPE numbers_table,
           END OF gear_type.
    TYPES gears_table TYPE TABLE OF gear_type WITH KEY position.

    DATA(gears) = VALUE gears_table( ).
    DATA(input) = SWITCH string( use_sample WHEN abap_true THEN get_puzzle_sample( ) ELSE get_puzzle_input( ) ).
    DATA(line_width) = SWITCH i( use_sample WHEN abap_true THEN 10 ELSE 140 ).
    DATA(line_count) = strlen( input ) DIV line_width.
    DATA(offset) = 0.
    WHILE offset < line_count * line_width.
      ...
      DATA(positions) = VALUE integer_array( FOR row = row_begin WHILE row <= row_while
                                             ...
                                             ( row * line_width + col ) ).
      LOOP AT positions INTO DATA(position).
        IF NOT input+position(1) = '*'.
          CONTINUE.
        ENDIF.
        IF NOT line_exists( gears[ position = position ] ).
          INSERT VALUE gear_type( position = position ) INTO TABLE gears.
        ENDIF.
        DATA(numbers) = VALUE numbers_table( BASE gears[ position = position ]-numbers
                                             ( number ) ).
        gears[ position = position ]-numbers = numbers.
      ENDLOOP.
    ENDWHILE.
    ...
    result = REDUCE i(
      ...
      FOR gear IN gears
      NEXT sum = sum
        + COND i( WHEN lines( gear-numbers ) = 2
                  THEN REDUCE i(
                       INIT ratio = 1
                       FOR <number> IN gear-numbers
                       NEXT ratio = ratio * <number> )
                  ELSE 0 ) ).
    WRITE / |{ result } is the sum of all of the gear ratios in your engine schematic.|.
  ENDMETHOD.
