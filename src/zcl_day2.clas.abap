CLASS zcl_day2 DEFINITION PUBLIC FINAL CREATE PUBLIC.
  PUBLIC SECTION.
...
CLASS zcl_day2 IMPLEMENTATION.
  METHOD part1.
    TYPES: BEGIN OF cubes,
             count TYPE i,
             color TYPE c LENGTH 5,
           END OF cubes.
    TYPES cubes_table TYPE TABLE OF cubes WITH KEY color.

    DATA(cubes) = VALUE cubes_table( ( count = 12 color = 'red' )
                                     ( count = 13 color = 'green' )
                                     ( count = 14 color = 'blue' ) ).
    DATA(lines) = SWITCH string_table( use_sample WHEN abap_true THEN get_puzzle_sample( ) ELSE get_puzzle_input( ) ).
    LOOP AT lines INTO DATA(line).
      SPLIT line AT ': ' INTO DATA(header) DATA(details).
      SPLIT replace( val  = details
                     sub  = ';'
                     with = ','
                     occ  = 0 ) AT ', ' INTO TABLE DATA(subsets).

      LOOP AT subsets INTO DATA(subset).
        SPLIT subset AT ' ' INTO DATA(count) DATA(color).
        DATA(cube_limit) = cubes[ color = color ]-count.
        DATA(cube_count) = CONV i( count ).
        IF cube_count > cube_limit.
          WRITE / |{ cube_count }/{ cube_limit } [{ color }] { line }|.
          EXIT.
        ENDIF.
        IF sy-tabix = lines( subsets ).
          result = result + CONV i( header+5 ). " Game 12" -> "12" -> 12
          WRITE / |OK { line }|.
        ENDIF.
      ENDLOOP.
    ENDLOOP.
  ...
  METHOD part2.
    ...
    result = 0.
    LOOP AT lines INTO DATA(line).
      DATA(cubes) = VALUE cubes_table( count = 0
                                       ( color = 'red' )
                                       ( color = 'green' )
                                       ( color = 'blue' ) ).
      SPLIT line AT ': ' INTO DATA(header) DATA(details).
      ...
      LOOP AT subsets INTO DATA(subset).
        SPLIT subset AT ' ' INTO DATA(count) DATA(color).
        DATA(cube_count) = CONV i( count ).
        IF cubes[ color = color ]-count < cube_count.
          cubes[ color = color ]-count = cube_count.
        ENDIF.
      ENDLOOP.
      result = result + REDUCE i( INIT power = 1 FOR cube IN cubes NEXT power = power * cube-count ).
    ENDLOOP.
    WRITE / result.
  ENDMETHOD.
